...
namespace Weibo.Config
{
    ...
    /// 基本设置信息管理类
    /// </summary>
    class BaseConfigFileManager : Weibo.Config.DefaultConfigFileManager
    {
        ...
        public new static string ConfigFilePath
        {
            get
            {
                if (filename == null)
                {
                    ...
                    if (context != null)
                    {
                        filename = context.Server.MapPath("~/Weibo.config");
                    ...
                    {
                        filename = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Weibo.config");
                    }

                    if (!File.Exists(filename))
                    {
                        throw new WeiboException("发生错误: 虚拟目录或网站根目录下没有正确的Weibo.config文件");
                    }
                }
                return filename;
            }
