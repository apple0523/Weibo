...
namespace Weibo.Config
{
    ...
    ///  Email配置管理类
    /// </summary>
    class EmailConfigFileManager : Weibo.Config.DefaultConfigFileManager
    {
        ...
        public new static string ConfigFilePath
        {
            get
            {
                if (filename == null)
                {
                    ...
                    if (context != null)
                    {
                        filename = context.Server.MapPath("~/Config/Email.config");
                    ...
                    {
                        filename = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Email.config");
                    }

                    if (!File.Exists(filename))
                    {
                        throw new WeiboException("发生错误: 虚拟目录或网站根目录下没有正确的Email.config文件");
                    }
                }
                return filename;
            }
